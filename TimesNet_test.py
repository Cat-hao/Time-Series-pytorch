...
configs = argparse.ArgumentParser(description='Bluetooth RSSI Predict Distance')
...
configs.add_argument('--dropout', type=float, default=0.5,
                     help='the dropout rate of the network')
configs.add_argument('--num_class', type=int, default=3)
configs = configs.parse_args()

model = Model(configs)
model.load_state_dict(torch.load('./TimesNet_models/TimesNet_model_epoch_404.pth'))
model.eval()
